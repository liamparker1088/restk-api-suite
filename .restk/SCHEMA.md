...
Everything here reflects the current format. Follow it exactly as written below.
...
A request's **folder is its directory location** — there is no parent-id
field. Move the file to re-parent the request.

## Common conventions

- **id** — a unique ID, always a **quoted string**. Never change it.
...
- **Rows** (variables, headers, params) are compact flow-mappings:
  ...
  — `enabled` defaults true (omitted when true); `secret: true` means the
  value is held by the app and never written here, so `value` is omitted.
...
```yaml
# Restk Request
id: "410001"                       # unique ID, quoted, DO NOT CHANGE
name: Create User
...
headers:
    ...
    - {id: "h2", key: X-Api-Key, order: 20, secret: true}   # value stays in the app
...
variables:
    ...
    - {id: "22", key: api_token, order: 20, secret: true}   # value stays in the app
```
